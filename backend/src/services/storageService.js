...
async function upload(buffer, filename, mimetype) {
  if (STORAGE === "s3") {
    ...
    // S3_PUBLIC_URL doit déjà pointer sur le bucket (ex. R2 dev URL pub-xxx.r2.dev).
    // Si seul S3_ENDPOINT est défini, on append le bucket (API S3 standard MinIO/AWS).
