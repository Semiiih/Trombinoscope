...
import { getClasses, getStudents, deleteStudent, uploadPhoto } from "../api/client";
...
export default function Students() {
  ...
  function openEdit(s: Student) { setEditing(s); setShowModal(true); }

  async function confirmDelete() {
    if (!confirmTarget) return;
    try { await deleteStudent(confirmTarget.id); load(); }
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try { await uploadPhoto(photoTarget.id, e.target.files[0]); load(); }
    catch (err) {
  ...
  const selectedClass = filterClass ? classes.find((c) => c.id === Number(filterClass)) : null;

  return (
    <div className="min-h-screen bg-slate-50 p-8">
      <div className="flex items-center justify-between mb-10">
        <div>
          <Breadcrumb parent="Gestion" parentPath="/classes" current={selectedClass?.label} />
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Élèves</h1>
          <p className="text-sm text-slate-400 mt-1">{students.length} élève{students.length !== 1 ? "s" : ""}</p>
        </div>
      ...
      <div className="flex gap-3 mb-6">
        ...
        <Select
          ...
          options={classes.map((c) => ({ value: String(c.id), label: `${c.label} — ${c.year}` }))}
        />
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <StudentsTable
          ...
          onPhotoUpload={(s) => { setPhotoTarget(s); fileRef.current?.click(); }}
        />
      </div>
